...
template<typename in_CType, typename in_BMAccessorType, 
  typename out_CType, typename out_BMAccessorType>
void IntMultiplyBM(BigMatrix *pInMat, BigMatrix *pOutMat, double value_)
{
  in_BMAccessorType inMat( *pInMat );
  out_BMAccessorType outMat( *pOutMat );
...
extern "C"
{
  ...
  #define CALL_intDivisionBM_2(IN_CTYPE, IN_ACCESSOR, OUT_ACCESSOR, value) \
    switch(pOutMat->matrix_type()) \
    { \
      case 4: \
        IntDivisionBM<IN_CTYPE, IN_ACCESSOR<IN_CTYPE>, int, OUT_ACCESSOR<int> >( \
          pInMat, pOutMat, value); \
        break; \
      case 8: \
        IntDivisionBM<IN_CTYPE, IN_ACCESSOR<IN_CTYPE>, double, OUT_ACCESSOR<double> >( \
          pInMat, pOutMat, value); \
        break; \
    }

  #define CALL_intDivisionBM_1(IN_ACCESSOR, OUT_ACCESSOR, value) \
    switch(pInMat->matrix_type()) \
    { \
      case 4: \
        CALL_intDivisionBM_2(int, IN_ACCESSOR, OUT_ACCESSOR, value) \
        break; \
      case 8: \
        CALL_intDivisionBM_2(double, IN_ACCESSOR, OUT_ACCESSOR, value) \
        break; \
    }
      
  SEXP CDivisionIntBM(SEXP inAddr, SEXP outAddr, SEXP value_, SEXP typecast_warning)
  {
    BigMatrix *pInMat = reinterpret_cast<BigMatrix*>(
      R_ExternalPtrAddr(inAddr));
    BigMatrix *pOutMat = reinterpret_cast<BigMatrix*>(
    ...
    int value = as<double>(value_);
    
    // Not sure if there is a better way to do these function calls
    if (pInMat->separated_columns() && pOutMat->separated_columns()) {
      CALL_intDivisionBM_1(SepMatrixAccessor, SepMatrixAccessor, value)
    }
    else if(pInMat->separated_columns() && !(pOutMat->separated_columns()))
    {
      CALL_intDivisionBM_1(SepMatrixAccessor, MatrixAccessor, value)
    }
    else if(!(pInMat->separated_columns()) && pOutMat->separated_columns())
    {
      CALL_intDivisionBM_1(MatrixAccessor, SepMatrixAccessor, value)
    }
    else
    {
      CALL_intDivisionBM_1(MatrixAccessor, MatrixAccessor, value)
    }
  ...
  #define CALL_intMultiplyBM_2(IN_CTYPE, IN_ACCESSOR, OUT_ACCESSOR, value) \
    ...
    { \
      case 4: \
        IntMultiplyBM<IN_CTYPE, IN_ACCESSOR<IN_CTYPE>, int, OUT_ACCESSOR<int> >( \
          pInMat, pOutMat, value); \
        break; \
      case 8: \
        IntMultiplyBM<IN_CTYPE, IN_ACCESSOR<IN_CTYPE>, double, OUT_ACCESSOR<double> >( \
          pInMat, pOutMat, value); \
        break; \
    }

  #define CALL_intMultiplyBM_1(IN_ACCESSOR, OUT_ACCESSOR, value) \
    switch(pInMat->matrix_type()) \
    { \
      case 4: \
        CALL_intMultiplyBM_2(int, IN_ACCESSOR, OUT_ACCESSOR, value) \
        break; \
      case 8: \
        CALL_intMultiplyBM_2(double, IN_ACCESSOR, OUT_ACCESSOR, value) \
        break; \
    }
      
  SEXP CMultiplyIntBM(SEXP inAddr, SEXP outAddr, SEXP value_, SEXP typecast_warning)
  {
    BigMatrix *pInMat = reinterpret_cast<BigMatrix*>(
      R_ExternalPtrAddr(inAddr));
    BigMatrix *pOutMat = reinterpret_cast<BigMatrix*>(
    ...
    // Not sure if there is a better way to do these function calls
    if (pInMat->separated_columns() && pOutMat->separated_columns()) {
      CALL_intMultiplyBM_1(SepMatrixAccessor, SepMatrixAccessor, value)
    }
    else if(pInMat->separated_columns() && !(pOutMat->separated_columns()))
    {
      CALL_intMultiplyBM_1(SepMatrixAccessor, MatrixAccessor, value)
    }
    else if(!(pInMat->separated_columns()) && pOutMat->separated_columns())
    {
      CALL_intMultiplyBM_1(MatrixAccessor, SepMatrixAccessor, value)
    }
    else
    {
